...
def enviarBucket(data, logProcesso):
    try:
        fetch_log = "http://localhost:3333/aws/logProcesso"
...
def obterProcessos(mac_address):
    try:
        fetch_pedido = f"http://localhost:3333/mysql/processoCliente/{mac_address}"
...
def excluirProcesso(mac_address, idProcesso, pid):
    try:
        fetch_excluirProcesso = f"http://localhost:3333/mysql/excluirProcesso/{mac_address}/{idProcesso}/{pid}"
